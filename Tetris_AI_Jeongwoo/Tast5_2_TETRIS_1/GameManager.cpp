...
#pragma region Fill_Data_to_String
		...
		for (int i = 4; i < displayManager->PLAY_AREA_HEIGHT; i++)
		{
			...
			{
				if (displayManager->GetScreen(i, j) == 1)//멈춘 블럭
				{
					screenBuffer[i - 4][j + 10] = "▣";
				}
				else if (displayManager->GetScreen(i, j) == 2)//움직이는 블럭
				{
					screenBuffer[i - 4][j + 10] = "■";
				}
				else//빈 공간
...
{
	if ((clock() - (float)t) / CLOCKS_PER_SEC >= speed)//CLOCKS_PER_SEC로 나눠줘야 초단위로 나온다.  
	{
		ResetTimer();//t = clock();
		return true;
	}
...
eInputKey GameManager::GetInputKey(bool bIsPlaying)
{
	while (true)
	{
		if (bIsPlaying && CheckTimePassed())//Clock() 발동!! 두개 다 트루라면
		{
